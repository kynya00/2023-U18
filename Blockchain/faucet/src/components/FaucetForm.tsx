...
export function FaucetForm() {
  const [{ status, ...state }, dispatch] = useReducer(
    (_: FaucetFormState, action: FaucetFormAction): FaucetFormState => {
  ...
  async function submit(address: string) {
    ...
    try {
      ...
      dispatch({ type: "error", message: "Something bad happened :(" });
    }
  }
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        submit(data.get("address") as string);
      ...
      {status === "error" && (
        ...
        >
          <span className="break-all">
            {(state as FaucetFormState & { status: "error" }).message}
          </span>
        </div>
      )}
      {status === "success" && (
